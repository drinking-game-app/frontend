...
const Home = (props: IProps & IActions) => {
  ...
  const [rejoinInfo, setRejoinInfo] = useState<IRejoinGame>({
    id: "",
    lobbyName: "",
  });
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("seenRules")
      .then((bool) => {
        if (!bool || bool !== "true") {
          AsyncStorage.setItem("seenRules", "true").catch((err) =>
            console.log("error setting seen rules", err)
          );
    ...
    if (props.token && props.token !== "")
      return (
        <View style={styles.signOutAndCogContainer}>
          ...
          <Button
            style={styles.settingsCog}
            onPress={() => props.navigation.navigate(AppRoute.RULES)}
            status="info"
  ...
  const renderRejoin = () => {
    if (canRejoin) {
      ...
      return (
        <Button
          style={styles.formButtonAlternate}
          onPress={() => {
            props.initGameSock();
            props
              .autoRejoinLobby(rejoinInfo)
              .then(() => {
  ...
  return (
    <Layout style={styles.container}>
      <ScrollView
        ...
        contentContainerStyle={styles.scrollChildContainer}
      >
